...
## REQUIRED CHAT SOURCE FILES (WAJIB)
- Before parsing any order range, you **MUST** read and audit both chat source files in the project root: `Chat1.txt` and `Chat2.txt`.
- Treat `Chat1.txt` and `Chat2.txt` as one combined chronological WhatsApp source, but do not assume either file is complete by itself.
- Always cross-check the requested date/time range against **both files** so no order is missed, duplicated, or taken from the wrong timestamp.
- If the files are named with spaces or alternate casing (for example `Chat 1.txt`, `Chat 2.txt`, `chat1.txt`, or `chat2.txt`), use the matching available pair and explicitly note which files were checked.
- If one file is missing, empty, or unreadable, stop and ask the user before finalizing the CSV because the output may be incomplete.
...
11. **CSV Output Ordering (CRITICAL)**:
   - Sort final order groups by `chatDate` from **oldest to newest** (ascending chronological order). The order closest to the start of the requested time range must appear at the top of the CSV.
   - Keep every item row belonging to the same customer/order consecutive. Do not split one order into separate parts while sorting.
   - When an order is revised or added to, use its latest message time as required above, then sort it at that latest `chatDate` position.
   - If a valid order has no timestamp, do **NOT** invent one. Put it after all rows with a valid `chatDate` and explicitly report that its timestamp still needs review.
...
- `ITS D 24` vs `BPD D 24/25`: A chat mentioning `D 24`, `D24`, `D/24`, or `ITS D 24` must map to `ITS D 24` (ongkir 5000). Do NOT map it to `BPD D 24/25` (ongkir 0) unless the chat explicitly contains the word `BPD` or the range `24/25` / `24-25` / `24 / 25`.
...
| **Astutik Perpus**                                        | "Astutik Perpus", "Astutik"                                                                                                                                                              | 5000                      |
...
| **Bu Nawir Mulyosari - MU 6/24**                          | "Bu Nawir Mulyosari - MU 6/24", "Nana/MU 6/24", "Nana MU 6/24"                                                                                                                           | 0                         |
...
| **ITS D 24**                                              | "ITS D 24", "ITS D24", "D 24", "D24", "D/24", "ITS D/24", "Blok D24"                                                                                                                      | 5000                      |
| **ITS F 6**                                               | "ITS F 6"                                                                                                                                                                                 | 0                         |
...
| **ITS T 8 LAMA**                                          | "ITS T 8 LAMA", "T 8 Lama", "T8 lm", "T8 lama"                                                                                                                                          | 5000                      |
...
| **Jl.Bhaskara 2 no 12**                                   | "Jl.Bhaskara 2 no 12", "Bojone Agung Gg2"                                                                                                                                               | 0                         |
...
| **Pantai Mentari Blok SF no. 9**                          | "Pantai Mentari Blok SF no. 9", "Pantai Mentari Blok SF / 9", "Pantai Mentri Blok SF / 9"                                                                                              | 10000                     |
...
| **Taman Mulyo Utara 7**                                  | "Taman Mulyo Utara 7", "Taman Mulyo Ut 7", "Taman Mulyo Ut 7 - Telly - Taman Mulyosari Utara no 7", "Telly", "Taman Mulyosari Utara no 7"                                           | 0                         |
...
| **Wisper 5/15**                                           | "Wisper 5/15", "Wisper 5 / 15"                                                                                                                                                            | 5000                      |
