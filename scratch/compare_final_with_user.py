...
user_csv_text = """customer,chatDate,payment,ongkir,item,quantity,note
...
# Parse user CSV
user_rows = []
for row in csv.DictReader(user_csv_text.strip().split('\n')):
...
# Helper key for basic order (ignoring customer name mapped details, matching strictly by datetime, item, quantity)
def make_basic_key(r):
    # Kita cari item murni, quantity, dan datetime/chatDate
    # Biar tahu apakah baris order ini beneran ada
    return (r['chatDate'], r['item'], r['quantity'])

user_keys = {make_basic_key(r): r for r in user_rows}
fixed_keys = {make_basic_key(r): r for r in fixed_rows}

print("=== BARIS DI USER CSV YANG TIDAK ADA DI FIXED CSV KITA ===")
missing_count = 0
for k, r in user_keys.items():
    if k not in fixed_keys:
        print(f"User Row -> Customer: {r['customer']} | Date: {r['chatDate']} | Item: {r['item']} ({r['quantity']})")
        missing_count += 1

print(f"Total baris yang hilang/beda key: {missing_count}")
