...
MENU_KEYWORDS = {
    "Rawon": ["rawon"],
    "Botok Uritan / 1": ["botok uritan", "bothok uritan", "botok urit", "bothok urit", "uritan", "bothok uritan / 1", "botok", "bothok"],
    "Kpl Ikan Mayung": [
        "kpl ikan mayung", "kepala ikan mayung", "kpl mayung", "kepala mayung", "mayung",
        "manyung", "kepala manyung", "kepala ikan manyung", "kpl manyung", "ikan manyung",
        "kepala ikan", "kpl ikan"
    ],
    "Sayur Bayem": ["sayur bayem", "sayur bayam", "bayem", "bayam"],
    "Bali Tahu Telur": ["bali tahu telur", "bali tahu telor", "bali tahu", "bali telur", "bali telor", "bali telur tahu", "bali telor tahu"],
    "Mendol Tempe MLG / 6": ["mendol tempe mlg / 6", "mendol tempe mlg", "mendol tempe", "mendol", "mendol mlg", "mendol tempe mlg /6", "mendoan tempe", "mendoan", "tempe mendoan"],
    "Lontong Kikil": ["lontong kikil", "kikil"],
    "Salad Buah": ["salad buah", "salad", "salat buah", "salat"],
    "Martabak Jadul": ["martabak jadul", "martabak"]
}

# Jam target (25 Juni 18:31 s.d. 26 Juni 12:00 siang)
...
if __name__ == '__main__':
    ...
    for msg in chats:
        ...
        # If no items and not a cancellation/address/delivery message, skip it
    ...
    for sender, msgs in orders_by_sender.items():
        ...
        # If it was a cancellation message and had no items, we start with empty items
        if current['is_cancel_msg'] and not current['items']:
            current['items'] = []
            
        for next_msg in msgs[1:]:
            # If the next message is a cancellation message and has no items, it cancels everything!
            ...
            # Otherwise, standard consolidation
            if not next_msg['items']:
                # Message has only address or delivery note
                current['address_lines'] = list(set(current['address_lines'] + next_msg['address_lines']))
                if next_msg['delivery_note']:
                    if current['delivery_note']:
                        if next_msg['delivery_note'] not in current['delivery_note']:
        ...
        # Split items in case of "Sebagian/Satu..." note rules
        current['items'] = split_order_items(current['items'])
        
        # Only keep if has items
        if current['items']:
            final_consolidated_orders.append(current)

    final_consolidated_orders.sort(key=lambda x: x['datetime'])

    output_path = '/Users/notslimboy/Documents/Cashier Web Apps/orders-2026-06-26.csv'
    ...
    with open(output_path, 'w', newline='', encoding='utf-8') as csvfile:
        ...
        for order in final_consolidated_orders:
            ...
            for item in order['items']:
                ...
                if matched_cust:
                    for addr in order['address_lines']:
                        if not is_duplicate_address(matched_cust['name'], addr):
